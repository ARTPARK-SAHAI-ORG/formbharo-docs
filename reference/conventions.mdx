...
All endpoints sit under `/api/v1` on your FormBharo server. For example, to list agents on a server at `https://YOUR-FORMBHARO-SERVER`, the full URL is `https://YOUR-FORMBHARO-SERVER/api/v1/agents`.

<Note>
  **Exception:** `POST /offer` (the WebRTC signalling endpoint) is at the bare path — `https://YOUR-FORMBHARO-SERVER/offer` — with no `/api/v1` prefix.
</Note>
