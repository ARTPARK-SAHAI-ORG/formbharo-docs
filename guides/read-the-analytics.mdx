...
```bash curl
curl -G "https://form-fill-agent-backend.artpark.ai/api/v1/agents/AGENT_ID/analytics" \
...
```bash curl
curl -G "https://form-fill-agent-backend.artpark.ai/api/v1/workspaces/WORKSPACE_ID/analytics" \
  -H "Authorization: Bearer <api-key>" \
  --data-urlencode "since=1700000000"
```
