...
All endpoints sit under `/api/v1` on your FormBharo server. For example, to list agents on a server at `https://form-fill-agent-backend.artpark.ai`, the full URL is `https://form-fill-agent-backend.artpark.ai/api/v1/agents`.

<Note>
  **Exception:** `POST /offer` (the WebRTC signalling endpoint) is at the bare path — `https://form-fill-agent-backend.artpark.ai/offer` — with no `/api/v1` prefix.
</Note>
