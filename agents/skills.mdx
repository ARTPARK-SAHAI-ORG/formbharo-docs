...
Set one environment variable, `FORMBHARO_API_KEY`. To get the key, open the web app, click your profile picture in the top right, then **API keys**. The key starts with `fb_live_` and is shown once.
...
It works from a plain-English description only, not a PDF or document directly. To turn a form document into an agent, see [Create an agent from a PDF](/guides/pdf-to-form).
