---
title: "Agent Config JSON Schema"
sidebarTitle: "Config Schema"
description: "Schema for the JSON view in the agent editor's edit page."
---

This is the format used by the "JSON view" toggle on the agent edit page. Paste a config in this shape, then Save.

<Note>
  This is not the raw API request body. [Agent Configuration](/reference/agent-configuration) documents `POST`/`PUT /api/v1/agents`, which uses `description`, `ai_instructions`, `question`, and `response_type`. The JSON view here uses `context`, `instructions`, `label`, and `type` for the same concepts.
</Note>
...
| `free_text` | any | `{ "type": "free_text", "rules": { "description": "Must be a real village name" } }` — note: this is guidance shown to the agent, not enforced on the value |
...
`free_text` is not enforced. The backend does not check the answer against `rules.description` at all — it's extra guidance text fed to the agent, not a validation check on the value.
