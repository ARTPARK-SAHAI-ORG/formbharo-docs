...
<Note>
  This is not the raw API request body. `POST`/`PUT /api/v1/agents` uses `description`, `ai_instructions`, `question`, and `response_type` for the same concepts the JSON view calls `context`, `instructions`, `label`, and `type`.
</Note>
