...
- `api-reference/openapi.json` is synced automatically from the live FormBharo
  server by `.github/workflows/sync-api-spec.yml`, on every production deploy
  and weekly as a backup. Do not hand-edit it.
- Every `{FORMBHARO_URL}` reference in a page reads from
  `snippets/formbharo-url.mdx`. Change the server domain there once, not in
  every page.
